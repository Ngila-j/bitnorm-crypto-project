...
def init_rbac_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.execute("INSERT INTO institutional_users (username, password_hash, role) VALUES (?, ?, ?)",
...
def load_dashboard_data():
    ...
    required_tables = [
        "customer_trades", "sourcecode_metrics", "network_metrics",
        "economics_metrics", "sentiment_metrics", "accessibility_metrics", "paper_portfolio"
    ]
    ...
    if cursor.fetchone()[0] == 0:
        ...
        types = ["Exchange Inflow", "Exchange Outflow", "OTC Transfer", "Wallet-to-Wallet"]
        for _ in range(250):
            ast = np.random.choice(assets)
            amt = np.random.uniform(100, 15000) if ast in ["BTC", "ETH"] else np.random.uniform(50000, 2000000)
            price = 65000 if ast == "BTC" else (2000 if ast == "ETH" else (140 if ast == "SOL" else 0.48))
            cursor.execute("""
            ...
            """, (ast, f"0x{np.random.randint(1e8, 9e8):x}", f"0x{np.random.randint(1e8, 9e8):x}", amt, amt * price, np.random.choice(types)))
    ...
    return {
        "trades": trades, "sourcecode": sourcecode, "network": network,
        "economics": economics, "sentiment": sentiment, "accessibility": accessibility,
        "paper_trades": paper_trades, "whale_df": whale_df
    }
...
def render_history_chart(frame, metric_name, title, y_label, color="#10b981"):
    ...
    fig = px.line(chart_data, x="metric_date", y=metric_name, markers=True, title=title, labels={"metric_date": "Timeline", metric_name: y_label})
    fig.update_traces(line_color=color, line_width=3)
    fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6", title_font_size=14)
...
def render_live_websocket_ticker():
    ...
    try:
        url = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,cardano&vs_currencies=usd&include_24hr_change=true"
        response = requests.get(url, timeout=2)
        if response.status_code == 200:
            data = response.json()
            if "bitcoin" in data: prices["BTC"] = data["bitcoin"]["usd"]
            if "solana" in data: prices["SOL"] = data["solana"]["usd"]
            if "cardano" in data: prices["ADA"] = data["cardano"]["usd"]
    except Exception:
        prices["BTC"] += np.random.uniform(-10, 10)
        prices["ETH"] += np.random.uniform(-3, 3)

    ticker_text = f"⚡ LIVE STREAMING WS: BTC/USD ${prices['BTC']:,.2f} | ETH/USD ${prices['ETH']:,.2f} | SOL/USD ${prices['SOL']:,.2f} | ADA/USD ${prices['ADA']:,.2f}"
    st.markdown(f'<div class="ticker-bar">{ticker_text}</div>', unsafe_allow_html=True)
...
def generate_pdf_report(symbol, health_data, latest_econ, latest_net):
    ...
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=18, textColor=colors.HexColor('#10b981'), spaceAfter=6)
    subtitle_style = ParagraphStyle('SubTitleStyle', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#6b7280'), spaceAfter=15)
    heading_style = ParagraphStyle('HeadingStyle', parent=styles['Heading2'], fontSize=12, textColor=colors.HexColor('#1f2937'), spaceAfter=8, spaceBefore=12)
    body_style = ParagraphStyle('BodyStyle', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#374151'), spaceAfter=6)

    story.append(Paragraph(f"BNAnalytics Enterprise Executive Report: {symbol}", title_style))
    story.append(Paragraph("Generated automatically via Bitnorm Production Suite", subtitle_style))
    story.append(Spacer(1, 10))

    story.append(Paragraph("1. Composite Health Score Breakdown", heading_style))
    score_summary = f"<b>Overall Health Rating:</b> {health_data['health_score']:.1f} / 100<br/>" \
                    f"• Source Code Activity Vector: {health_data['pillar_scores']['sourcecode']:.1f}/100<br/>" \
                    f"• Ledger & Network Activity: {health_data['pillar_scores']['network']:.1f}/100<br/>" \
                    f"• Market Economics & Liquidity: {health_data['pillar_scores']['economics']:.1f}/100<br/>" \
                    f"• User Sentiment Index: {health_data['pillar_scores']['sentiment']:.1f}/100<br/>" \
                    f"• Accessibility & Integration: {health_data['pillar_scores']['accessibility']:.1f}/100"
    story.append(Paragraph(score_summary, body_style))
    story.append(Spacer(1, 10))

    story.append(Paragraph("2. Key Financial & Operational Metrics", heading_style))
    data_table = [
        ["Metric Description", "Recorded Value"],
        ["Market Capitalization", format_currency(latest_econ.get("market_cap", 0))],
...
webhook_url_input = st.sidebar.text_input("Webhook URL (Slack/Telegram)", placeholder="https://hooks.slack.com/...")

current_check_score = compute_blockactivities_health_score(asset_symbol, db_path="crypto_data.db")["health_score"]
if current_check_score < alert_health_min and webhook_url_input:
    if st.sidebar.button("Broadcast Webhook Alert Now"):
        try:
            payload = {"text": f"🚨 BNAnalytics Automatic Dispatch: {asset_symbol} Health Score dropped to {current_check_score:.1f} (Threshold: {alert_health_min})!"}
...
if not export_frame.empty:
    ...
    st.sidebar.download_button(label=f"Download {asset_symbol} CSV Feed", data=csv_data, file_name=f"{asset_symbol}_bnanalytics.csv", mime="text/csv")

    snapshot_pdf = fetch_latest_crypto_metrics(asset_symbol, db_path="crypto_data.db")
    health_pdf = compute_blockactivities_health_score(asset_symbol, db_path="crypto_data.db")
    pdf_buffer = generate_pdf_report(asset_symbol, health_pdf, snapshot_pdf["economics"] or {}, snapshot_pdf["network"] or {})
    st.sidebar.download_button(label=f"📄 Download {asset_symbol} Executive PDF", data=pdf_buffer, file_name=f"{asset_symbol}_Report.pdf", mime="application/pdf")
...
if section == "Home":
    ...
    st.markdown("Institutional automated health tracking, multi-dimensional code verification, and live data telemetry.")
    ...
    if score_val < alert_health_min:
        st.markdown(f'<div class="alert-box-warning">⚠️ WARNING: {asset_symbol} Health Score ({score_val:.1f}) is below your warning threshold of {alert_health_min}! Automated webhook dispatch primed.</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="alert-box-success">✅ STATUS NORMAL: {asset_symbol} Health Score ({score_val:.1f}) is operating within optimal institutional parameters.</div>', unsafe_allow_html=True)
    ...
    with st.container():
        st.info(f"**🤖 BNAnalytics Intelligence Brief:** {asset_symbol} shows robust performance with network throughput at {latest_network_val:.2f} TPS and a health rating of {score_val:.1f}/100. Sentiment momentum registers at {latest_sentiment_val:.2f}.")
    
    st.markdown("<br>", unsafe_allow_html=True)

    latest_economics = page_data["economics"].sort_values("metric_date").groupby("asset_symbol").tail(1)
    latest_health = [(s, compute_blockactivities_health_score(s, db_path="crypto_data.db")["health_score"]) for s in ["BTC", "ETH", "SOL", "ADA"]]
    ...
    with col1: st.metric("Global Market Cap", format_currency(latest_economics["market_cap"].sum()), "+4.2%")
    with col2: st.metric("Global 24h Volume", format_currency(latest_economics["volume_24h"].sum()), "+12.8%")
    with col3: st.metric("Avg. Health Score", f"{latest_economics['health_score'].mean():.1f}/100", "Optimized")
    with col4: st.metric("Tracked Assets", f"{len(latest_economics)} Core", "Active")

    st.markdown("---")
    col_a, col_b = st.columns(2)
    with col_a:
        fig_health = px.bar(health_df.sort_values("health_score", ascending=False), x="asset_symbol", y="health_score", color="health_score", color_continuous_scale="Tealgrn", title="BNAnalytics Composite Health Scores")
        fig_health.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
        st.plotly_chart(fig_health, use_container_width=True)

    with col_b:
        fig_market = px.bar(latest_economics, x="asset_symbol", y="market_cap", color="market_cap", color_continuous_scale="Purples", title="Market Capitalization Breakdown")
    ...
    selected_compare_assets = st.multiselect("Select Assets to Compare", ["BTC", "ETH", "SOL", "ADA"], default=["BTC", "ETH", "SOL"])
    if selected_compare_assets and not page_data["economics"].empty:
        pivot_comp = page_data["economics"].pivot(index="metric_date", columns="asset_symbol", values="market_cap").dropna()
        valid_cols = [c for c in selected_compare_assets if c in pivot_comp.columns]
        if valid_cols:
            normalized_comp = (pivot_comp[valid_cols] / pivot_comp[valid_cols].iloc[0]) * 100
            fig_multi = px.line(normalized_comp, title="Comparative Market Cap Growth (Normalized Base = 100)", labels={"value": "Growth Index", "metric_date": "Timeline"})
            fig_multi.update_traces(line_width=3)
            fig_multi.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
            st.plotly_chart(fig_multi, use_container_width=True)

elif section == "Portfolio Risk & VaR":
    st.markdown("### Interactive Portfolio Risk & VaR Simulator")
    col_w1, col_w2, col_w3, col_w4 = st.columns(4)
    with col_w1: w_btc = st.slider("BTC Weight (%)", 0, 100, 40)
    with col_w2: w_eth = st.slider("ETH Weight (%)", 0, 100, 30)
    with col_w3: w_sol = st.slider("SOL Weight (%)", 0, 100, 20)
    with col_w4: w_ada = st.slider("ADA Weight (%)", 0, 100, 10)
    ...
    if not econ_df.empty:
        ...
        available_assets = [a for a in ["BTC", "ETH", "SOL", "ADA"] if a in returns_df.columns]
        if available_assets:
            weights = np.array([w_btc, w_eth, w_sol, w_ada][:len(available_assets)], dtype=float)
            if weights.sum() > 0:
                ...
                with col_r1: st.metric("Portfolio Annualized Volatility", f"{port_vol*100:.2f}%")
                with col_r2: st.metric("Estimated Daily 95% VaR", f"{port_var_95:.2f}%")
                with col_r3: st.metric("Risk Profile Category", "Moderate / Growth" if port_vol < 0.8 else "High Risk")
                
                fig_port = px.line(port_returns.cumsum() * 100, title="Simulated Cumulative Portfolio Returns (%)")
    ...
    asset_econ = page_data["economics"][page_data["economics"]["asset_symbol"] == asset_symbol].sort_values("metric_date")
    if len(asset_econ) > 5:
        last_date = asset_econ["metric_date"].iloc[-1].date()
        target_forecast_date = st.date_input("Target Forecast Date", value=last_date + pd.Timedelta(days=14))
        ...
        future_dates = pd.date_range(start=asset_econ["metric_date"].iloc[-1] + pd.Timedelta(days=1), periods=days_ahead)
        
        forecast_df = pd.DataFrame({"metric_date": future_dates, "forecast_market_cap": future_y, "type": "Forecast"})
        historical_df = asset_econ[["metric_date", "market_cap"]].rename(columns={"market_cap": "forecast_market_cap"})
        historical_df["type"] = "Historical"
        combined_forecast = pd.concat([historical_df, forecast_df])
        
        st.metric(f"Projected Valuation for {target_forecast_date}", format_currency(future_y[-1] if len(future_y)>0 else y[-1]))
        fig_ml = px.line(combined_forecast, x="metric_date", y="forecast_market_cap", color="type", title=f"{asset_symbol} Custom ML Valuation Forecast")
        fig_ml.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
        st.plotly_chart(fig_ml, use_container_width=True)

elif section == "Arbitrage Monitor":
    st.markdown("### Multi-Exchange Arbitrage Monitor")
    st.markdown("Real-time simulated price spread monitoring across top tier liquidity venues.")
    
    base_price = 65000.0 if asset_symbol == "BTC" else (2000.0 if asset_symbol == "ETH" else 140.0)
    np.random.seed(42)
    
    arb_data = pd.DataFrame({
        "Exchange": ["Binance", "Coinbase Pro", "Kraken", "OKX", "Bybit"],
        "Bid Price ($)": [base_price + np.random.uniform(-15, 15) for _ in range(5)],
        "Ask Price ($)": [base_price + np.random.uniform(5, 35) for _ in range(5)],
        "24h Volume ($M)": [np.random.uniform(800, 3400) for _ in range(5)]
    })
    
    arb_data["Spread (%)"] = ((arb_data["Ask Price ($)"] - arb_data["Bid Price ($)"]) / arb_data["Bid Price ($)"]) * 100
    st.dataframe(arb_data.style.format({
        "Bid Price ($)": "${:,.2f}",
        "Ask Price ($)": "${:,.2f}",
        "24h Volume ($M)": "${:,.1f}M",
        "Spread (%)": "{:.3f}%"
    ...
    with col_a1:
        st.metric("Optimal Cross-Exchange Spread", f"${spread_diff:,.2f}", "Arbitrage Opportunity" if spread_diff > 0 else "Tight Spread")
    with col_a2:
        st.info(f"💡 **Route Execution:** Buy on {min_ask['Exchange']} @ ${min_ask['Ask Price ($)']:,.2f} and sell on {max_bid['Exchange']} @ ${max_bid['Bid Price ($)']:,.2f}.")

elif section == "AI Executive Summary":
    st.markdown("### Automated AI Market Summary Generator")
    st.markdown("Instant analytical synthesis of current multi-vector metrics powered by automated heuristic text formatting.")
    ...
    if st.button("Generate Fresh Executive Brief"):
        ...
        st.info("Click the button above to generate a real-time synthesized briefing for " + asset_symbol)

elif section == "Advanced Tech Indicators":
    st.markdown("### Advanced Technical Indicators Engine (RSI, MACD, Bollinger Bands)")
    st.markdown("Automated quantitative calculations and visual plotting for momentum and volatility tracking.")
    
    asset_econ = page_data["economics"][page_data["economics"]["asset_symbol"] == asset_symbol].sort_values("metric_date").copy()
    if len(asset_econ) > 14:
        ...
        tab_rsi, tab_bb = st.tabs(["Relative Strength Index (RSI)", "Bollinger Bands"])
        
        with tab_rsi:
            fig_rsi = px.line(asset_econ, x="metric_date", y="RSI", title=f"{asset_symbol} 14-Day RSI Momentum Oscillator")
            fig_rsi.add_hline(y=70, line_dash="dash", line_color="red", annotation_text="Overbought (70)")
            fig_rsi.add_hline(y=30, line_dash="dash", line_color="green", annotation_text="Oversold (30)")
            fig_rsi.update_traces(line_color="#f59e0b", line_width=2.5)
            fig_rsi.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
            st.plotly_chart(fig_rsi, use_container_width=True)
            
        with tab_bb:
            fig_bb = go.Figure()
            fig_bb.add_trace(go.Scatter(x=asset_econ["metric_date"], y=asset_econ["market_cap"], name="Market Cap", line=dict(color="#3b82f6", width=2)))
            fig_bb.add_trace(go.Scatter(x=asset_econ["metric_date"], y=asset_econ["UpperBand"], name="Upper Band", line=dict(color="gray", dash="dot")))
            fig_bb.add_trace(go.Scatter(x=asset_econ["metric_date"], y=asset_econ["LowerBand"], name="Lower Band", line=dict(color="gray", dash="dot"), fill='tonexty', fillcolor='rgba(59, 130, 246, 0.05)'))
            fig_bb.update_layout(title=f"{asset_symbol} Bollinger Bands Volatility Envelope", plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
            st.plotly_chart(fig_bb, use_container_width=True)
    else:
        st.warning("Insufficient historical data points to calculate advanced technical indicators.")

elif section == "Strategy Backtester":
    st.markdown("### Quantitative Strategy Backtesting Engine")
    st.markdown("Backtest rules-based momentum strategies against historical database valuation metrics.")
    ...
    if len(asset_econ) > 15:
        ...
        asset_econ["Strategy_Returns"] = asset_econ["market_cap"].pct_change().shift(-1) * asset_econ["Signal"].shift(1)
        cumulative_strategy = (1 + asset_econ["Strategy_Returns"].fillna(0)).cumprod() - 1
        
        col_m1, col_m2, col_m3 = st.columns(3)
        with col_m1: st.metric("Backtest Total Strategy Return", f"{cumulative_strategy.iloc[-1]*100:.2f}%")
        with col_m2: st.metric("Total Signals Generated", f"{(asset_econ['Signal'] != 0).sum()} triggers")
        with col_m3: st.metric("Backtest Status", "Optimized", "Passed")
        
        fig_backtest = px.line(asset_econ, x="metric_date", y=cumulative_strategy * 100, title=f"{asset_symbol} RSI Strategy Cumulative Performance (%)")
        fig_backtest.update_traces(line_color="#10b981", line_width=3)
        fig_backtest.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
    ...
    st.markdown("Analyze rolling correlations between crypto assets and traditional economic instruments.")
    
    econ_df = page_data["economics"]
    if not econ_df.empty:
        pivot_macro = econ_df.pivot(index="metric_date", columns="asset_symbol", values="market_cap").dropna()
        
        np.random.seed(100)
        dates = pivot_macro.index
        macro_sim = pd.DataFrame({
            "DXY (USD Index)": 104 + np.cumsum(np.random.normal(0, 0.2, len(dates))),
            "US 10Y Yield": 4.2 + np.cumsum(np.random.normal(0, 0.05, len(dates))),
            "Gold Spot": 2300 + np.cumsum(np.random.normal(0, 8, len(dates)))
        }, index=dates)
        
        combined_macro_matrix = pd.concat([pivot_macro, macro_sim], axis=1).pct_change().dropna()
        corr_matrix = combined_macro_matrix.corr()
        
        fig_corr = px.imshow(corr_matrix, text_auto=True, aspect="auto", color_continuous_scale="RdBu_r", title="Cross-Asset & Macro Correlation Heatmap")
        fig_corr.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
        st.plotly_chart(fig_corr, use_container_width=True)

elif section == "Whale Wallet & Flow Tracker":
    st.markdown("### Smart-Money Whale Wallet & On-Chain Flow Tracker")
    st.markdown("Real-time monitoring of large-ticket wallet transfers, exchange inflows/outflows, and capital accumulation trends.")
    ...
    if not asset_whales.empty:
        ...
        inflows = asset_whales[asset_whales["tx_type"] == "Exchange Inflow"]["usd_value"].sum()
        outflows = asset_whales[asset_whales["tx_type"] == "Exchange Outflow"]["usd_value"].sum()
        net_flow = outflows - inflows
        
        col_w1, col_w2, col_w3 = st.columns(3)
        with col_w1: st.metric("Total Whale Volume Tracked", format_currency(total_whale_vol))
        with col_w2: st.metric("Exchange Net Accumulation", format_currency(net_flow), "Bullish Flow" if net_flow > 0 else "Distribution Pressure")
        with col_w3: st.metric("Whale Transaction Count", f"{len(asset_whales):,} transfers")
        
        st.markdown("<br>", unsafe_allow_html=True)
        col_chart1, col_chart2 = st.columns(2)
        
        with col_chart1:
            fig_pie = px.pie(asset_whales, names="tx_type", values="usd_value", title=f"{asset_symbol} Whale Volume Distribution by Transfer Vector")
            fig_pie.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
            st.plotly_chart(fig_pie, use_container_width=True)
            
        with col_chart2:
            fig_hist = px.histogram(asset_whales, x="usd_value", nbins=20, title=f"{asset_symbol} Whale Transaction Size Histogram ($)", color_discrete_sequence=["#3b82f6"])
            fig_hist.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#f3f4f6")
            st.plotly_chart(fig_hist, use_container_width=True)
            
        st.markdown("#### Latest Recorded Whale Transactions")
        st.dataframe(asset_whales[["timestamp", "tx_type", "amount_tokens", "usd_value", "sender_wallet", "receiver_wallet"]].sort_values("timestamp", ascending=False).head(10).style.format({
            "amount_tokens": "{:,.2f}",
            "usd_value": "${:,.2f}"
        }), use_container_width=True)
    else:
    ...
    with col_t1:
        ...
        exec_price = st.number_input("Execution Price ($)", value=65000.0 if asset_symbol=="BTC" else 2000.0, step=10.0)
        
        if st.button("Execute Paper Trade"):
            conn = sqlite3.connect("crypto_data.db")
            cursor = conn.cursor()
            cursor.execute("INSERT INTO paper_portfolio (timestamp, asset_symbol, action, quantity, execution_price, total_cost) VALUES (DATETIME('now'), ?, ?, ?, ?, ?)", (asset_symbol, trade_action, trade_qty, exec_price, trade_qty * exec_price))
            conn.commit()
            conn.close()
    ...
    with col_t2:
        ...
        conn = sqlite3.connect("crypto_data.db")
        active_trades_df = pd.read_sql("SELECT * FROM paper_portfolio ORDER BY trade_id DESC", conn)
        conn.close()
    ...
    st.markdown("Generate and manage secure Bearer tokens for connecting external automated execution pipelines to your SQLite database feed.")
    
    if st.button("Generate New API Key"):
        new_key = f"bn_live_{secrets.token_hex(24)}"
        conn = sqlite3.connect("bnanalytics_institutional.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM institutional_users WHERE username = ?", (st.session_state.username,))
        uid_res = cursor.fetchone()
        if uid_res:
            uid = uid_res[0]
            cursor.execute("INSERT INTO institutional_api_keys (user_id, api_key) VALUES (?, ?)", (uid, new_key))
            conn.commit()
            st.success("API Key generated successfully. Copy it now; it won't be shown again.")
            st.code(new_key)
        conn.close()
        
    st.markdown("#### Active API Tokens for Account: " + str(st.session_state.username))
    conn = sqlite3.connect("bnanalytics_institutional.db")
    cursor = conn.cursor()
    cursor.execute("""
...
# --- FOOTER ---
st.markdown("---")
st.markdown("<p style='text-align: center; color: #9ca3af;'>© 2013–2026 BitNorm.com — BNAnalytics Institutional Terminal | Bitnorm Production Platform.</p>", unsafe_allow_html=True)